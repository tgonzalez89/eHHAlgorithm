...
void ComputeNetwork(
	...
	cellState local_state[IO_NETWORK_SIZE],	
	mod_prec iAppin[IO_NETWORK_SIZE],
	//int N_Size,
	int Mux_Factor,
	mod_prec Connectivity_Matrix[CONN_MATRIX_SIZE],
	//int Conn_Matrix_Size,
	mod_prec cellOut[IO_NETWORK_SIZE]
){
#pragma HLS INTERFACE s_axilite port=Connectivity_Matrix bundle=bus
#pragma HLS INTERFACE s_axilite port=return bundle=bus
#pragma HLS INTERFACE s_axilite port=local_state bundle=bus
#pragma HLS INTERFACE s_axilite port=iAppin bundle=bus
#pragma HLS INTERFACE s_axilite port=Mux_Factor bundle=bus
#pragma HLS INTERFACE s_axilite port=cellOut bundle=bus

	int i,j,k;

	//cellState local_state[IO_NETWORK_DIM1*IO_NETWORK_DIM2][TIME_MUX_FACTOR];
	//cellState local_state0[MAX_TIME_MUX];
	cellState new_state[MAX_TIME_MUX];
	//mod_prec IAppin0[MAX_TIME_MUX];
	//mod_prec Connectivity_Matrix0[CONN_MATRIX_MAX];
	...
	//		local_state0[j]= IniArray[j+(Mux_Factor*0)];
	//	}
	//}
	...
	//	ComputeNetwork_connectivity:for (j=0;j<Conn_Matrix_Size;j++){

	//		Connectivity_Matrix0[j] = Connectivity_Matrix[j];
	//	}
	//}
	...
	ComputeNetwork_label2:	for(i=0;i<Mux_Factor;i++){
		...
		neighVdend[k] = local_state[i].dend.V_dend ;
		k=k+HW_CELLS;
	}
	...
	ComputeOneCellTimeMux0_label1: for(j=0;j<Mux_Factor;j++){
		prevCellState = local_state[j];
    	//The three compartments can be computed concurrently but only across a single sim step
		new_state[j].dend = CompDend(prevCellState.dend, prevCellState.soma.V_soma , iAppin[j], neighVdend,MAX_N_SIZE,Connectivity_Matrix,j);
		new_state[j].soma = CompSoma(prevCellState.soma,prevCellState.dend.V_dend, prevCellState.axon.V_axon);
		new_state[j].axon = CompAxon(prevCellState.axon ,prevCellState.soma.V_soma);
	...
	ComputeNetwork_label3:	for(j=0;j<Mux_Factor;j++){
	...
	ComputeNetwork_label4:	for(j=0;j<Mux_Factor;j++){


		cellOut[j+(TIME_MUX_FACTOR*0)] = local_state[j].axon.V_axon;
...
Dend CompDend(Dend prevDend, mod_prec prevSoma , mod_prec iAppIn, mod_prec neighVdend[MAX_N_SIZE], int N_Size,mod_prec Connectivity_Matrix[CONN_MATRIX_MAX],int j){
...
mod_prec DendKCurr(struct channelParams chPrms){
    ...
    alpha_s = ((0.00002*prevCa2Plus) < 0.01) ? (0.00002*prevCa2Plus) : 0.01;
...
mod_prec IcNeighbors(mod_prec neighVdend[MAX_N_SIZE], mod_prec prevV_dend, int N_Size ,mod_prec Connectivity_Matrix[CONN_MATRIX_MAX],int j){
...
IcNeighbors_label0:	for(i=0;i<N_Size;i++){
						...
						//f = 0.8 * expf(-1*V*V/100) + 0.2;    // SCHWEIGHOFER 2004 VERSION
