...
void Network_CalcNextState(Neuron network[], fpn I_app_array[], fpn ConnectivityMatrix[][NETWORK_SIZE]) {
    ...
    for (size_t i = 0; i < NETWORK_SIZE; i++) {
        ...
        network[i].dend = Dend_CalcNextState(network[i].dend, prevV_soma, I_app_array[i], NeighV_dend, ConnectivityMatrix[i]);
...
Dend Dend_CalcNextState(Dend dend, fpn V_soma, fpn I_app, fpn NeighV_dend[], fpn Connectivity[]) {
    fpn_pair temp;
    ...
    fpn I_c = Dend_CalcI_cNeighbors(dend.V_dend, NeighV_dend, Connectivity);
...
fpn Dend_CalcI_cNeighbors(fpn V_dend, fpn NeighV_dend[], fpn Connectivity[]) {
